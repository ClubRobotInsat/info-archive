...
class Log {
public:
	enum Output { TERMINAL, TERMINAL_NO_COLOR, TXT, HTML, RTF };
...
public:
	...
	static void setThreadName(const std::string& name) {
		thread_infos[glfwGetThreadID()].name = name;
	}
	static std::string getThreadName() {
		return thread_infos[glfwGetThreadID()].name;
	}
#endif
	...
	template <class T0>
	static void write(LogType type, const char* file_path, int line, const char* function_name, const T0& arg0) {
		write(type, file_path, line, function_name, arg0, "", "", "", "", "", "", "", "", "", "");
	}

	template <class T0, class T1>
	static void write(LogType type, const char* file_path, int line, const char* function_name, const T0& arg0, const T1& arg1) {
		write(type, file_path, line, function_name, arg0, arg1, "", "", "", "", "", "", "", "", "");
	}

	template <class T0, class T1, class T2>
	static void
	    write(LogType type, const char* file_path, int line, const char* function_name, const T0& arg0, const T1& arg1, const T2& arg2) {
		write(type, file_path, line, function_name, arg0, arg1, arg2, "", "", "", "", "", "", "", "");
	}

	template <class T0, class T1, class T2, class T3>
	static void
	    write(LogType type, const char* file_path, int line, const char* function_name, const T0& arg0, const T1& arg1, const T2& arg2, const T3& arg3) {
...
void Log::write(LogType type,
	...
	switch(type) {
		case LOG_INFO:
			ss << "[" << file_name << ":" << line << " in " << function_name << "] : " << arg0 << arg1 << arg2 << arg3
			   << arg4 << arg5 << arg6 << arg7 << arg8 << arg9 << arg10 << std::flush;
		...
		case LOG_FAILED:
			...
			break;
		case LOG_WARN:
			ss << "[" << file_name << ":" << line << " in " << function_name << "] : WARNING : " << arg0 << arg1 << arg2
			   << arg3 << arg4 << arg5 << arg6 << arg7 << arg8 << arg9 << arg10 << std::flush;
			msg_first_part = ss.str();
			doFormatting(&msg_first_part, LOG_WARN, Log::output);
			break;
		case LOG_ERROR:
			ss << "[" << file_name << ":" << line << " in " << function_name << "] : ERROR : " << arg0 << arg1 << arg2
			   << arg3 << arg4 << arg5 << arg6 << arg7 << arg8 << arg9 << arg10 << std::flush;
			msg_first_part = ss.str();
		...
		default: // LOG_DEBUG_X
			ss << "[" << file_name << ":" << line << " in " << function_name << "] : DEBUG " << int(type) - int(LOG_DEBUG_0)
...
// If GLFW threads are used, write the thread's name if not "", followed by its ID.
#ifdef LOG_ENABLE_GLFW_THREADS
	{
		...
		if(it != thread_infos.end() && it->second.name != "")
	...
	if(Log::output == TERMINAL) {
		...
		(*p_stream) << indentation << msg_first_part << msg_second_part << " <br/>"
		            << "</span>" << std::flush;
