	...
	*found = 0;
	...
	for(i = 1; i <= count; i++) {
		...
		if(_glfwWin.has_WGL_ARB_pixel_format) {
			...
			if(!getPixelFormatAttrib(i, WGL_SUPPORT_OPENGL_ARB) || !getPixelFormatAttrib(i, WGL_DRAW_TO_WINDOW_ARB) ||
			...
			if(getPixelFormatAttrib(i, WGL_ACCELERATION_ARB) == WGL_NO_ACCELERATION_ARB) {
				continue;
			}
			...
			if(!(pfd.dwFlags & PFD_DRAW_TO_WINDOW) || !(pfd.dwFlags & PFD_SUPPORT_OPENGL) || !(pfd.dwFlags & PFD_DOUBLEBUFFER)) {
				// Only consider doublebuffered OpenGL pixel formats for windows
				continue;
			}
...
static void translateChar(DWORD wParam, DWORD lParam, int action) {
	...
	if(_glfwLibrary.Sys.hasUnicode) {
		num_chars = ToUnicode(wParam,         // virtual-key code
		                      scan_code,      // scan code
		...
		num_chars = ToAscii(wParam,           // virtual-key code
		                    scan_code,        // scan code
		                    keyboard_state,   // key-state array
...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch(uMsg) {
		...
		case WM_MOUSEMOVE: {
			...
			if(NewMouseX != _glfwInput.OldMouseX || NewMouseY != _glfwInput.OldMouseY) {
				if(_glfwWin.mouseLock) {
					_glfwInput.MousePosX += NewMouseX - _glfwInput.OldMouseX;
					_glfwInput.MousePosY += NewMouseY - _glfwInput.OldMouseY;
				} else {
...
static void getFullWindowSize(int clientWidth, int clientHeight, int* fullWidth, int* fullHeight) {
	RECT rect;
...
static void initWGLExtensions(void) {
	...
	_glfwWin.GetExtensionsStringEXT = (PFNWGLGETEXTENSIONSSTRINGEXTPROC)wglGetProcAddress("wglGetExtensionsStringEXT");
	if(!_glfwWin.GetExtensionsStringEXT) {
		_glfwWin.GetExtensionsStringARB =
	...
	if(_glfwPlatformExtensionSupported("WGL_EXT_swap_control")) {
		...
		_glfwWin.SwapIntervalEXT = (PFNWGLSWAPINTERVALEXTPROC)wglGetProcAddress("wglSwapIntervalEXT");
	}

	if(_glfwPlatformExtensionSupported("WGL_ARB_pixel_format")) {
		_glfwWin.has_WGL_ARB_pixel_format = GL_TRUE;
		_glfwWin.GetPixelFormatAttribivARB =
...
static int createWindow(const _GLFWwndconfig* wndconfig, const _GLFWfbconfig* fbconfig) {
...
int _glfwPlatformOpenWindow(int width, int height, const _GLFWwndconfig* wndconfig, const _GLFWfbconfig* fbconfig) {
	...
	if(_glfwWin.fullscreen) {
		_glfwSetVideoMode(
		    &_glfwWin.width, &_glfwWin.height, fbconfig->redBits, fbconfig->greenBits, fbconfig->blueBits, wndconfig->refreshRate);
	}
	...
	if(wndconfig->glMajor > 2) {
		if(wndconfig->glMajor != _glfwWin.glMajor || wndconfig->glMinor != _glfwWin.glMinor) {
	...
	if(_glfwWin.fullscreen) {
		...
		SetWindowPos(_glfwWin.window, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
	}
...
void _glfwPlatformSetWindowSize(int width, int height) {
	...
	if(_glfwWin.fullscreen && (width > _glfwWin.width)) {
		SetWindowPos(_glfwWin.window, HWND_TOP, 0, 0, width, height, SWP_NOOWNERZORDER | SWP_NOMOVE | SWP_NOZORDER);
		sizechanged = GL_TRUE;
	}
...
void _glfwPlatformSetWindowPos(int x, int y) {
	(void)SetWindowPos(_glfwWin.window, HWND_TOP, x, y, 0, 0, SWP_NOOWNERZORDER | SWP_NOSIZE | SWP_NOZORDER);
}
...
void _glfwPlatformRefreshWindowParams(void) {
	...
	if(_glfwWin.has_WGL_ARB_pixel_format) {
		if(getPixelFormatAttrib(pixelFormat, WGL_ACCELERATION_ARB) != WGL_NO_ACCELERATION_ARB) {
		...
		_glfw_DescribePixelFormat(_glfwWin.DC, pixelFormat, sizeof(PIXELFORMATDESCRIPTOR), &pfd);

		// Is current OpenGL context accelerated?
		_glfwWin.accelerated = (pfd.dwFlags & PFD_GENERIC_ACCELERATED) || !(pfd.dwFlags & PFD_GENERIC_FORMAT) ? 1 : 0;
