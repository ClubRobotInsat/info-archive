...
enum { _GLFW_COND_SIGNAL = 0, _GLFW_COND_BROADCAST = 1 };
...
GLFWcond _glfwPlatformCreateCond(void) {
	...
	cond->events[_GLFW_COND_SIGNAL] = CreateEvent(NULL, FALSE, FALSE, NULL);
	cond->events[_GLFW_COND_BROADCAST] = CreateEvent(NULL, TRUE, FALSE, NULL);
...
void _glfwPlatformWaitCond(GLFWcond cond, GLFWmutex mutex, double timeout) {
	...
	EnterCriticalSection(&cv->waiters_count_lock);
	cv->waiters_count--;
	last_waiter = (result == WAIT_OBJECT_0 + _GLFW_COND_BROADCAST) && (cv->waiters_count == 0);
	LeaveCriticalSection(&cv->waiters_count_lock);
