	...
	if(nCode == HC_ACTION) {
		...
		switch(wParam) {
			...
			case WM_SYSKEYUP:
				...
				syskeys = (p->vkCode == VK_TAB && p->flags & LLKHF_ALTDOWN) ||
				          (p->vkCode == VK_ESCAPE && p->flags & LLKHF_ALTDOWN) ||
				          (p->vkCode == VK_F4 && p->flags & LLKHF_ALTDOWN) ||
				          (p->vkCode == VK_ESCAPE && (GetKeyState(VK_CONTROL) & 0x8000)) || p->vkCode == VK_LWIN ||
				          p->vkCode == VK_RWIN || p->vkCode == VK_APPS;
				break;
...
void _glfwPlatformDisableSystemKeys(void) {
	...
	if(_glfwLibrary.Sys.winVer >= _GLFW_WIN_NT4) {
		...
		_glfwWin.keyboardHook = SetWindowsHookEx(WH_KEYBOARD_LL, keyboardHook, _glfwLibrary.instance, 0);
	} else {
