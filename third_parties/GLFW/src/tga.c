...
static int ReadTGAHeader(_GLFWstream* s, _tga_header_t* h) {
	...
	h->_alphabits = (int)(h->imageinfo & _TGA_IMAGEINFO_ALPHA_MASK) >> _TGA_IMAGEINFO_ALPHA_SHIFT;
	h->_origin = (int)(h->imageinfo & _TGA_IMAGEINFO_ORIGIN_MASK) >> _TGA_IMAGEINFO_ORIGIN_SHIFT;
...
int _glfwReadTGA(_GLFWstream* s, GLFWimage* img, int flags) {
	...
	// Is there a colormap?
	cmapsize = (h.cmaptype == _TGA_CMAPTYPE_PRESENT ? 1 : 0) * h.cmaplen * ((h.cmapentrysize + 7) / 8);
	if(cmapsize > 0) {
	...
	if((swapy && !(flags & GLFW_ORIGIN_UL_BIT)) || (!swapy && (flags & GLFW_ORIGIN_UL_BIT))) {
		src = pix;
