...
static void removeJoystick(_glfwJoystick* joystick) {
	...
	if(joystick->present) {
		...
		for(i = 0; i < joystick->numAxes; i++) {
			_glfwJoystickElement* axes = (_glfwJoystickElement*)CFArrayGetValueAtIndex(joystick->axes, i);
			free(axes);
		}
	...
	for(i = 0; i < GLFW_JOYSTICK_LAST + 1; i++) {
		...
		if(joystick->present) {
			for(j = 0; j < joystick->numButtons; j++) {
				_glfwJoystickElement* button = (_glfwJoystickElement*)CFArrayGetValueAtIndex(joystick->buttons, j);
				button->value = getElementValue(joystick, button);
			}

			for(j = 0; j < joystick->numAxes; j++) {
				_glfwJoystickElement* axes = (_glfwJoystickElement*)CFArrayGetValueAtIndex(joystick->axes, j);
				axes->value = getElementValue(joystick, axes);
			}

			for(j = 0; j < joystick->numHats; j++) {
				_glfwJoystickElement* hat = (_glfwJoystickElement*)CFArrayGetValueAtIndex(joystick->hats, j);
...
void _glfwInitJoysticks(void) {
	...
	while((ioHIDDeviceObject = IOIteratorNext(objectIterator))) {
		...
		result = IOCreatePlugInInterfaceForService(
		    ioHIDDeviceObject, kIOHIDDeviceUserClientTypeID, kIOCFPlugInInterfaceID, &ppPlugInInterface, &score);
		...
		plugInResult =
		    (*ppPlugInInterface)
		        ->QueryInterface(ppPlugInInterface, CFUUIDGetUUIDBytes(kIOHIDDeviceInterfaceID), (void*)&(joystick->interface));
...
int _glfwPlatformGetJoystickButtons(int joy, unsigned char* buttons, int numbuttons) {
	...
	const int directions[9] = {1, 3, 2, 6, 4, 12, 8, 9, 0}; // Bit fields of button presses for each direction,
	                                                        // including nil
