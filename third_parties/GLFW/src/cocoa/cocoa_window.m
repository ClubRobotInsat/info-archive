...
int _glfwPlatformOpenWindow(int width, int height, const _GLFWwndconfig* wndconfig, const _GLFWfbconfig* fbconfig) {
	...
	if(wndconfig->mode == GLFW_FULLSCREEN) {
		fullscreenMode =
		    ...
		    CGDisplayBestModeForParametersAndRefreshRateWithProperty(CGMainDisplayID(),
		                                                             colorBits + fbconfig->alphaBits,
		                                                             width,
		                                                             height,
		                                                             wndconfig->refreshRate,
		                                                             // Controversial, see macosx_fullscreen.m for
		                                                             // discussion
	...
	_glfwWin.window = [[NSWindow alloc] initWithContentRect:NSMakeRect(0, 0, width, height)
	                                              styleMask:styleMask
	...
	if(wndconfig->mode == GLFW_FULLSCREEN) {
		_glfwLibrary.originalMode = (NSDictionary*)CGDisplayCurrentMode(CGMainDisplayID());
...
#define MAX_ATTRS 64 // urrgh
	...
	int accumBits = fbconfig->accumRedBits + fbconfig->accumGreenBits + fbconfig->accumBlueBits + fbconfig->accumAlphaBits;
	...
	_glfwWin.context = [[NSOpenGLContext alloc] initWithFormat:_glfwWin.pixelFormat shareContext:nil];
	...
	if(wndconfig->mode == GLFW_FULLSCREEN) {
		...
		[[_glfwWin.window contentView] enterFullScreenMode:[NSScreen mainScreen] withOptions:nil];
	}
...
void _glfwPlatformCloseWindow(void) {
	...
	if(_glfwWin.fullscreen) {
		...
		CGDisplaySwitchToMode(CGMainDisplayID(), (CFDictionaryRef)_glfwLibrary.originalMode);
...
void _glfwPlatformSetWindowPos(int x, int y) {
	...
	contentRect.origin =
	    NSMakePoint(screenRect.origin.x + x, screenRect.origin.y + screenRect.size.height - y - contentRect.size.height);
...
void _glfwPlatformSetMouseCursorPos(int x, int y) {
	if(_glfwWin.fullscreen) {
		...
		CGPoint targetPoint =
		    CGPointMake(globalPoint.x - mainScreenOrigin.x, mainScreenHeight - globalPoint.y - mainScreenOrigin.y);
