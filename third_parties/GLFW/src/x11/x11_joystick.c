	...
	for(k = 0; k <= 1 && joy_count <= GLFW_JOYSTICK_LAST; ++k) {
		...
		for(i = 0; i <= 50 && joy_count <= GLFW_JOYSTICK_LAST; ++i) {
			...
			if(fd != -1) {
				...
				_glfwJoy[joy_count].Axis = (float*)malloc(sizeof(float) * _glfwJoy[joy_count].NumAxes);
				if(_glfwJoy[joy_count].Axis == NULL) {
					close(fd);
					continue;
				}
				_glfwJoy[joy_count].Button = (unsigned char*)malloc(sizeof(char) * _glfwJoy[joy_count].NumButtons);
			...
			while(read(_glfwJoy[i].fd, &e, sizeof(struct js_event)) > 0) {
				...
				switch(e.type) {
					...
					case JS_EVENT_BUTTON:
						_glfwJoy[i].Button[e.number] = e.value ? GLFW_PRESS : GLFW_RELEASE;
						break;
