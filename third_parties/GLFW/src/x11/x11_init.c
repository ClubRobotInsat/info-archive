...
static void initLibraries(void) {
	...
	int i;
	char* libGL_names[] = {"libGL.so", "libGL.so.1", "/usr/lib/libGL.so", "/usr/lib/libGL.so.1", NULL};
...
// Check for XF86VidMode extension
	...
	_glfwLibrary.XF86VidMode.available =
	    XF86VidModeQueryExtension(_glfwLibrary.display, &_glfwLibrary.XF86VidMode.eventBase, &_glfwLibrary.XF86VidMode.errorBase);
#else
...
// Check for XRandR extension
	...
	_glfwLibrary.XRandR.available =
	    XRRQueryExtension(_glfwLibrary.display, &_glfwLibrary.XRandR.eventBase, &_glfwLibrary.XRandR.errorBase);
#else
	...
	if(!glXQueryVersion(_glfwLibrary.display, &_glfwLibrary.glxMajor, &_glfwLibrary.glxMinor)) {
