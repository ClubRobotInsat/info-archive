...
static unsigned long getWindowProperty(Window window, Atom property, Atom type, unsigned char** value) {
	...
	XGetWindowProperty(_glfwLibrary.display, window, property, 0, LONG_MAX, False, type, &actualType, &actualFormat, &itemCount, &bytesAfter, value);
...
static GLboolean checkForEWMH(void) {
	...
	Atom supportingWmCheck = XInternAtom(_glfwLibrary.display, "_NET_SUPPORTING_WM_CHECK", True);
	Atom wmSupported = XInternAtom(_glfwLibrary.display, "_NET_SUPPORTED", True);
	if(supportingWmCheck == None || wmSupported == None) {
		return GL_FALSE;
	}

	// Then we look for the _NET_SUPPORTING_WM_CHECK property of the root window
	if(getWindowProperty(_glfwWin.root, supportingWmCheck, XA_WINDOW, (unsigned char**)&windowFromRoot) != 1) {
	...
	if(getWindowProperty(*windowFromRoot, supportingWmCheck, XA_WINDOW, (unsigned char**)&windowFromChild) != 1) {
		XFree(windowFromRoot);
	...
	atomCount = getWindowProperty(_glfwWin.root, wmSupported, XA_ATOM, (unsigned char**)&supportedAtoms);
...
static _GLFWfbconfig* getFBConfigs(unsigned int* found) {
	...
	*found = 0;
	...
	if(strcmp(glXGetClientString(_glfwLibrary.display, GLX_VENDOR), "Chromium") == 0) {
	...
	if(_glfwWin.has_GLX_SGIX_fbconfig) {
		fbconfigs = _glfwWin.ChooseFBConfigSGIX(_glfwLibrary.display, _glfwWin.screen, NULL, &count);
		if(!count) {
	...
	for(i = 0; i < count; i++) {
		if(!getFBConfigAttrib(fbconfigs[i], GLX_DOUBLEBUFFER) || !getFBConfigAttrib(fbconfigs[i], GLX_VISUAL_ID)) {
	...
	if(_glfwWin.has_GLX_ARB_create_context) {
		...
		if(wndconfig->glProfile) {
			if(!_glfwWin.has_GLX_ARB_create_context_profile) {
				fprintf(stderr,
				        "OpenGL profile requested but GLX_ARB_create_context_profile "
				        "is unavailable\n");
				return GL_FALSE;
			}
...
static void initGLXExtensions(void) {
	...
	if(_glfwPlatformExtensionSupported("GLX_EXT_swap_control")) {
		_glfwWin.SwapIntervalEXT = (PFNGLXSWAPINTERVALEXTPROC)_glfwPlatformGetProcAddress("glXSwapIntervalEXT");
	...
	if(_glfwPlatformExtensionSupported("GLX_MESA_swap_control")) {
		_glfwWin.SwapIntervalMESA = (PFNGLXSWAPINTERVALMESAPROC)_glfwPlatformGetProcAddress("glXSwapIntervalMESA");
	...
	if(_glfwPlatformExtensionSupported("GLX_SGI_swap_control")) {
		_glfwWin.SwapIntervalSGI = (PFNGLXSWAPINTERVALSGIPROC)_glfwPlatformGetProcAddress("glXSwapIntervalSGI");
	...
	if(_glfwPlatformExtensionSupported("GLX_SGIX_fbconfig")) {
		_glfwWin.GetFBConfigAttribSGIX =
		    (PFNGLXGETFBCONFIGATTRIBSGIXPROC)_glfwPlatformGetProcAddress("glXGetFBConfigAttribSGIX");
		_glfwWin.ChooseFBConfigSGIX =
		    (PFNGLXCHOOSEFBCONFIGSGIXPROC)_glfwPlatformGetProcAddress("glXChooseFBConfigSGIX");
		_glfwWin.CreateContextWithConfigSGIX =
		    (PFNGLXCREATECONTEXTWITHCONFIGSGIXPROC)_glfwPlatformGetProcAddress("glXCreateContextWithConfigSGIX");
		_glfwWin.GetVisualFromFBConfigSGIX =
		    (PFNGLXGETVISUALFROMFBCONFIGSGIXPROC)_glfwPlatformGetProcAddress("glXGetVisualFromFBConfigSGIX");

		if(_glfwWin.GetFBConfigAttribSGIX && _glfwWin.ChooseFBConfigSGIX && _glfwWin.CreateContextWithConfigSGIX &&
		   _glfwWin.GetVisualFromFBConfigSGIX) {
	...
	if(_glfwPlatformExtensionSupported("GLX_ARB_create_context")) {
		_glfwWin.CreateContextAttribsARB =
		    (PFNGLXCREATECONTEXTATTRIBSARBPROC)_glfwPlatformGetProcAddress("glXCreateContextAttribsARB");
...
static GLboolean createWindow(int width, int height, const _GLFWwndconfig* wndconfig) {
	XEvent event;
	...
	_glfwWin.colormap = XCreateColormap(_glfwLibrary.display, _glfwWin.root, _glfwWin.visual->visual, AllocNone);
	...
	// Create the actual window
	{
		...
		_glfwWin.window = XCreateWindow(_glfwLibrary.display,
		                                _glfwWin.root,
		                                0,
		                                0, // Upper left corner of this window on root
		                                _glfwWin.width,
		                                _glfwWin.height,
		                                0,                      // Border width
	...
	if(_glfwWin.fullscreen && !_glfwWin.hasEWMH) {
		...
		XChangeWindowAttributes(_glfwLibrary.display, _glfwWin.window, CWOverrideRedirect, &attributes);

		_glfwWin.overrideRedirect = GL_TRUE;
	}

	// Find or create the protocol atom for window close notifications
	_glfwWin.wmDeleteWindow = XInternAtom(_glfwLibrary.display, "WM_DELETE_WINDOW", False);
...
static void enterFullscreenMode(void) {
	if(!_glfwWin.Saver.changed) {
		...
		XSetScreenSaver(_glfwLibrary.display, 0, 0, DontPreferBlanking, DefaultExposures);
	...
	if(_glfwWin.hasEWMH && _glfwWin.wmState != None && _glfwWin.wmStateFullscreen != None) {
		if(_glfwWin.wmActiveWindow != None) {
			...
			XSendEvent(_glfwLibrary.display, _glfwWin.root, False, SubstructureNotifyMask | SubstructureRedirectMask, &event);
		}
		...
		XSetInputFocus(_glfwLibrary.display, _glfwWin.window, RevertToParent, CurrentTime);
		XMoveWindow(_glfwLibrary.display, _glfwWin.window, 0, 0);
		XResizeWindow(_glfwLibrary.display, _glfwWin.window, _glfwWin.width, _glfwWin.height);
	}
	...
	XWarpPointer(_glfwLibrary.display, None, _glfwWin.window, 0, 0, 0, 0, _glfwWin.width / 2, _glfwWin.height / 2);
}
...
static GLboolean processSingleEvent(void) {
	...
	switch(event.type) {
			...
			if(event.xmotion.x != _glfwInput.CursorPosX || event.xmotion.y != _glfwInput.CursorPosY) {
				...
				if(_glfwWin.mouseLock) {
					if(_glfwWin.pointerHidden) {
						_glfwInput.MousePosX += event.xmotion.x - _glfwInput.CursorPosX;
						_glfwInput.MousePosY += event.xmotion.y - _glfwInput.CursorPosY;
					}
				} else {
		...
		case ConfigureNotify: {
			if(event.xconfigure.width != _glfwWin.width || event.xconfigure.height != _glfwWin.height) {
		...
		case ClientMessage: {
			if((Atom)event.xclient.data.l[0] == _glfwWin.wmDeleteWindow) {
				...
				return GL_TRUE;
			} else if(_glfwWin.wmPing != None && (Atom)event.xclient.data.l[0] == _glfwWin.wmPing) {
				// The window manager is pinging us to make sure we are still
				// responding to events

				event.xclient.window = _glfwWin.root;
				XSendEvent(_glfwLibrary.display, event.xclient.window, False, SubstructureNotifyMask | SubstructureRedirectMask, &event);
			}
...
int _glfwPlatformOpenWindow(int width, int height, const _GLFWwndconfig* wndconfig, const _GLFWfbconfig* fbconfig) {
	...
	if(wndconfig->mode == GLFW_FULLSCREEN) {
		...
		if(_glfwLibrary.XRandR.available) {
			XRRSelectInput(_glfwLibrary.display, _glfwWin.window, RRScreenChangeNotifyMask);
		}
#endif
		enterFullscreenMode();
	}
...
void _glfwPlatformRefreshWindowParams(void) {
	...
	if(fbconfig == NULL) {
		...
		fprintf(stderr,
		        "Cannot find known GLXFBConfig by ID. "
...
// Retrieve refresh rate if possible
	...
	if(_glfwLibrary.XF86VidMode.available) {
		...
		XF86VidModeGetModeLine(_glfwLibrary.display, _glfwWin.screen, &dotclock, &modeline);
...
void _glfwPlatformHideMouseCursor(void) {
	...
	if(!_glfwWin.pointerGrabbed) {
		if(XGrabPointer(_glfwLibrary.display,
		                _glfwWin.window,
		                True,
