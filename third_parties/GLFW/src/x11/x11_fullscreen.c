...
int _glfwGetClosestVideoMode(int screen, int* width, int* height, int* rate) {
	...
	if(_glfwLibrary.XRandR.available) {
		sc = XRRGetScreenInfo(_glfwLibrary.display, RootWindow(_glfwLibrary.display, screen));
		...
		for(i = 0; i < sizecount; i++) {
			match = (*width - sizelist[i].width) * (*width - sizelist[i].width) +
	...
	if(_glfwLibrary.XF86VidMode.available) {
		...
		XF86VidModeGetAllModeLines(_glfwLibrary.display, screen, &modecount, &modelist);

		// Find the best matching mode
		bestmode = -1;
		bestmatch = INT_MAX;
		for(i = 0; i < modecount; i++) {
			match = (*width - modelist[i]->hdisplay) * (*width - modelist[i]->hdisplay) +
...
void _glfwSetVideoModeMODE(int screen, int mode, int rate) {
	...
	if(_glfwLibrary.XRandR.available) {
		...
		if(rate > 0) {
			...
			XRRSetScreenConfigAndRate(_glfwLibrary.display, sc, root, mode, RR_Rotate_0, (short)rate, CurrentTime);
		} else {
			// Set desired configuration
			XRRSetScreenConfig(_glfwLibrary.display, sc, root, mode, RR_Rotate_0, CurrentTime);
		}
	...
	if(_glfwLibrary.XF86VidMode.available) {
		...
		XF86VidModeSwitchToMode(_glfwLibrary.display, screen, modelist[mode]);
...
void _glfwRestoreVideoMode(void) {
	if(_glfwWin.FS.modeChanged) {
		...
		if(_glfwLibrary.XRandR.available) {
			...
			XRRSetScreenConfig(_glfwLibrary.display, sc, _glfwWin.root, _glfwWin.FS.oldSizeID, _glfwWin.FS.oldRotation, CurrentTime);
		...
		if(_glfwLibrary.XF86VidMode.available) {
			...
			XF86VidModeSwitchToMode(_glfwLibrary.display, _glfwWin.screen, &_glfwWin.FS.oldMode);
		}
#endif
