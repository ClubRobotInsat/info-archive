...
Table generate_table() {
	...
	table.emplace(std::make_shared<Element>(ElementType::SPHERE, Coordonnees({0_m, 0_m})));
	table.emplace(std::make_shared<Element>(ElementType::SPHERE, Coordonnees({1_m, 2_m})));
...
MagicStrategy generate_strategy(Table table) {
	...
	auto action_sphere = [&action_cube](Coordonnees coords) -> Action {
		return Action(20_s, 80, Element(ElementType::SPHERE, coords), {Element(ElementType::CUBE, Coordonnees({2_m, 1_m}, 0_deg))}, "sphere");
	};
	...
	strategy.associate_element(ElementType::CUBE, action_cube, always_impossible);
	strategy.associate_element(ElementType::SPHERE, action_sphere, always_possible);
...
TEST_CASE("StrategyGenerator") {
	SECTION("DecisionalTree") {
		...
		tree.add_edge(tree.get_root(),
		              generate_table(),
		              Action(0_s, 0, Element(ElementType::NOTHING, Coordonnees({0_m, 1_m})), {}, "first_wait_action"));
		...
		CHECK(e->get_type() == ElementType::SPHERE);

		Action associated_action(20_s,
		                         80,
		                         *e,
		                         {Element(ElementType::CUBE, Coordonnees({1.5_m, 1_m})),
		                          Element(ElementType::CUBE, Coordonnees({2_m, 1_m}))},
		                         "sphere");
	...
	SECTION("Action") {
		SECTION("get_execution_time") {
			auto generator = [](Duration execution_time, repere::Coordonnees start_coords) -> Action {
				return Action(execution_time, 0, Element(ElementType::NOTHING, start_coords), {}, "action");
			};
