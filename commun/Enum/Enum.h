...
std::enable_if_t<std::is_enum<Enum>::value, std::underlying_type_t<Enum>> constexpr enumToInt(Enum e) {
 ...
 *&).
 * Ne convient pas pour déclarer une enum dans un namespace, pour cela utiliser les macros ENUM_NS et ENUM_CLASS_NS
...
/**
 * Exactement la même chose qu'au dessus, mis à part que l'enum est désormais une enum class (fortement typée, englobée
 * dans sa propre portée).
 */
 ...
 * Ne gère qu'un seul niveau de namespace, i.e namespace NS1 { namespace NS2 { --- déclaration ici --- } } ne peut pas
 *fonctionner (erreur de compilation)
 *
...
namespace {
	inline void _enum_f1() {
		MonEnum e =
			MonEnum::Foo; // MonEnum est une enum class comme une autre, on en manipule les valeurs comme n'importe
						  // quelle autre enum
		...
		for(auto ee : getEnumValues<MonEnum>()) {
			std::cout << ee << " : " << enumToInt(ee)
					  << std::endl; // Affiche 3 lignes : "Foo : 0", "Bar : 1" et "Baz : 2".
		}
	}


	// Test de MonEnum définie dans le namespace NS (définie avec ENUM_CLASS_NS(NS, …))
	inline void _enum_f2() {
		NS::MonEnum e =
			NS::MonEnum::Batman; // NS::MonEnum est une enum class comme une autre, on en manipule les valeurs
								 // comme n'importe quelle autre enum
		...
		std::string cppString =
			toString(NS::MonEnum::NaNaNa); // Création à partie de la chaîne partagée, modification à volonté ensuite

		// getEnumValues<NS::MonEnum>() récupère toutes les valeur de l'enum NS::MonEnum
		for(auto ee : getEnumValues<NS::MonEnum>()) {
			std::cout
				<< ee << " : " << enumToInt(ee)
				<< std::endl; // Affiche 5 lignes : "Na : 0", "NaNa : 1", "NaNaNa : 2", "NaNaNaNa : 3" et "Batman : 4".
		}
	}
}
