...
// Negation
template <class T>
Transform<T> Transform<T>::operator-() const {
	return Transform<T>(-this->rotation, -this->position);
}
...
void Transform<T>::rotate(Vector3<T_scalar> axis, T_scalar2 value) {
	T_scalar c = fastCos(float(value));
	T_scalar ic = 1 - c;
...
void Transform<T>::lookAt(Vector3<T_scalar> target) {
	...
	// rotation along Y
	if(axis.y <= 0) {
		if((Vector3<T>(0.0, 0.0, -1.0) * projected) >= 0)
			rotate(Vector3<T>(0.0, 1.0, 0.0), fastAsin(axis.norm()));
		else if((Vector3<T>(0.0, 0.0, -1.0) * projected) <= 0)
...
template <class T>
template <class T_scalar>
void Transform<T>::lookAtZup(Vector3<T_scalar> target) {
	// Calculus for the first angle
	Vector3<T> direction = (target - position);
	direction.normalize();
	rotation.setIdentity();
	rotate(Vector3f(1.0, 0.0, 0.0), 90);

	Vector3<T> projected(direction.x, direction.y, 0.0);
	...
	// Calculus for the second rotation
	projected.normalize();
	Vector3<T> axis = projected ^ (Vector3<T>(0.0, 1.0, 0.0));

	// rotation along Z
	if(axis.z <= 0) {
		if((Vector3<T>(0.0, 1.0, 0.0) * projected) >= 0)
			rotate(Vector3<T>(0.0, 0.0, 1.0), fastAsin(axis.norm()));
		else if((Vector3<T>(0.0, 1.0, 0.0) * projected) <= 0)
			rotate(Vector3<T>(0.0, 0.0, 1.0), 180.0 - fastAsin(axis.norm()));
	} else {
		if((Vector3<T>(0.0, 1.0, 0.0) * projected) >= 0)
			rotate(Vector3<T>(0.0, 0.0, 1.0), -fastAsin(axis.norm()));
		else if((Vector3<T>(0.0, 1.0, 0.0) * projected) <= 0)
			rotate(Vector3<T>(0.0, 0.0, 1.0), fastAsin(axis.norm()) - 180.0);
	}
}
