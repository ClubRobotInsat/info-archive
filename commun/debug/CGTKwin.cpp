...
void NDebug::boolean_fn(GtkWidget *pWidget, gpointer data){
	...
	if( ptr == nullptr ) return; // ( on sait jamais )
...
#ifdef GTKWIN_NO_GLFW
	pthread_create(&thread, nullptr, caller, nullptr);
#else
	thread = glfwCreateThread(caller, nullptr);
#endif
}
...
void NDebug::_CGTKwin::Init() {
	// std::cout << "GTKwin OK";
	if(memberGTKwin != nullptr) return; // membreGTKwin ne doit pas encore exister !! si c'est le cas, la fenêtre a déjà été initialisée et on n'a plus rien à faire.
	...
	pScrolledWin = gtk_scrolled_window_new(nullptr,nullptr);
...
#ifdef GTKWIN_TRACE
	g_signal_connect(G_OBJECT(pWindow), "destroy", G_CALLBACK(quit_fn), nullptr);
	gtk_table_set_row_spacings(GTK_TABLE(pTable),1);
	//gtk_container_add(GTK_CONTAINER(pWindow), GTK_WIDGET(pTable));
#endif
	gtk_window_set_title(GTK_WINDOW(pWindow), "Fenêtre Debug");
	idle_handler_id = gtk_idle_add(idle_fn, nullptr); // Ajout d'une fonction à appeler régulièrement
}
...
void NDebug::CGTKwin::Attach(bool& ptr, const std::string& txt) {
	...
	gchar* sUtf8 = g_locale_to_utf8(txt.c_str(), -1, nullptr, nullptr, nullptr);
...
void NDebug::_CGTKwin::Log(std::string str){
	...
	// gchar* sUtf8 = g_locale_to_utf8(str.c_str(),str.length() , nullptr, nullptr , nullptr);
	// if(sUtf8[str.length()]=='\n')sUtf8[str.length()-1]='\n';
