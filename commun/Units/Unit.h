...
public:
	/**
	 * Le type destiné à stocker les valeurs numériques de la grandeur
	 * (float, double, long double voire même entier en fonction de la précision voulue)
	 */
	 ...
	 * En revanche, il doit correspondre au type trait std::is_arithmetic.
	 *
	 * Cette fonction template peut être spécialisée pour autoriser la multiplication de deux grandeurs physiques non homogènes.
	 *
	 * @param v1 le premier facteur du produit
	 * @param v2 le deuxième facteur du produit
	 * @return une nouvelle instance représentant la multiplication de l'instance courante par le paramètre
	 */
	template<typename U>
	 ...
	 * En revanche, il doit correspondre au type trait std::is_arithmetic.
	 *
	 * Cette fonction template peut être spécialisée pour autoriser la division de deux grandeurs physiques non homogènes.
	 *
	 * @param v1 le dividende de la division
	 * @param v2 le diviseur de la division
	 * @return une nouvelle instance représentant la division de l'instance courante par le paramètre
	 */
	template<typename U>
	constexpr friend std::enable_if_t<std::is_arithmetic<U>::value, T>
	operator/(T const &v1, U v2) {
	...
	/**
	 * Divise l'instance courante par une instance de la même grandeur physique.
	 * La valeur retournée représente le rapport entre les deux grandeurs.
	 * @param v1 le dividende de la division
	 * @param v2 le diviseur de la division
	 * @return le rapport entre les deux grandeurs
	 */
	 ...
	 */
	constexpr friend T operator%(T const & v1, T v2) {
		return makeValue(std::fmod(v1._val, v2._val));
	}
	 ...
	 */
	constexpr friend T abs(T const &val) {
	 ...
	 */
	constexpr friend ValueType cos(T const &val) {
	 ...
	 */
	constexpr friend ValueType sin(T const &val) {
...
protected:
	/**
	 * Constructeur acceptant une valeur numérique, inaccessible depuis l'extérieur pour éviter le changement non désirée de grandeur physique.
	 */
