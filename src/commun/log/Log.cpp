...
void Log::writeInternal(LogType type, Output outputIndex, std::stringstream& stream) {
	...
	switch(type) {
		case LogType::INFO:
			doFormatting(msg_first_part, LogType::INFO, outputIndex);
			break;
		case LogType::SUCCESS:
			doFormatting(msg_first_part, LogType::INFO, outputIndex);

			msg_second_part = "[OK]";
			doFormatting(msg_second_part, LogType::SUCCESS, outputIndex);
			break;
		case LogType::FAILED:
			doFormatting(msg_first_part, LogType::INFO, outputIndex);

			msg_second_part = "[FAILED]";
			doFormatting(msg_second_part, LogType::FAILED, outputIndex);
			break;
		case LogType::WARN:
			doFormatting(msg_first_part, LogType::WARN, outputIndex);
			break;
		case LogType::ERROR:
			doFormatting(msg_first_part, LogType::ERROR, outputIndex);
			break;

		default: // LogType::DEBUG_X
