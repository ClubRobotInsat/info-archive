...
void Display();
void TreatData(const unsigned short* input_data, unsigned char* displayed_data, int w, int h);
...
int main(int argc, char* argv[]) {
	...
	if(!glfwOpenWindow(webcam->GetDepthWidth(), webcam->GetDepthHeight(), 0, 0, 0, 0, 0, 0, GLFW_WINDOW)) {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, webcam->GetDepthWidth(), webcam->GetDepthHeight(), 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
	...
	frames = 0;
	t0 = glfwGetTime();
	buffer_data = new unsigned char[webcam->GetDepthWidth() * webcam->GetDepthHeight() * 3];
...
void Display() {
	...
	int w = webcam->GetDepthWidth(), h = webcam->GetDepthHeight();
	...
	// Traitement
	TreatData(webcam->GetDepthData(), buffer_data, w, h);
...
void TreatData(const unsigned short* input_data, unsigned char* displayed_data, int w, int h) {
	const unsigned short* p_in = input_data;
	unsigned char* p_out = displayed_data;
	for(int y = 0; y < h; y++) {
		for(int x = 0; x < w; x++, p_in++, p_out += 3) {
			*(p_out + 0) = 255; // R
			*(p_out + 1) = 0;   // G
			*(p_out + 2) = 0;   // B
		}
