...
vector<ImageFiltree::Tache> tachesHautBleu;
vector<ImageFiltree::Tache> tachesBasBleu;
...
int main(int argc, char* argv[]) {
	...
	//	for (int i=0; i<NB_COULEURS_HAUT; i++) couleurTachesHautBleu[i] = UNDEFINED;
	//	for (int i=0; i<NB_COULEURS_BAS; i++) couleurTachesBasBleu[i] = UNDEFINED;
	nbTachesBasBleu = 0;
	nbTachesHautBleu = 0;
	nbTachesBas = 0;
	nbTachesHaut = 0;
...
void Display() {
	...
	for(int i = 0; i < nbTachesHaut; i++)
		printf("+ x=%d, y=%d\n", tachesHaut.at(i).x, tachesHaut.at(i).y);

	// Taches du BAS
	printf("DEBUG : taches du bas \n");
	nbTachesBas = tachesBas.size();
	for(int i = 0; i < nbTachesBas; i++)
		printf("+ x=%d, y=%d\n", tachesBas.at(i).x, tachesBas.at(i).y);

	if(isBlue) {
		// Copie des données dans des tableaux pour les garder plus tard
		tachesBasBleu = tachesBas;
		tachesHautBleu = tachesHaut;
		//		for (int i=0; i<NB_COULEURS_HAUT; i++)
		...
		//// HAUT ////
		printf("Calcul des tâches pour le HAUT\n");
		nbTachesHautBleu = tachesHautBleu.size();
		nbTachesHaut = tachesHaut.size();
		bool isConsistent = (nbTachesHaut + nbTachesHautBleu == NB_COULEURS_HAUT);
		int indexB = 0, indexR = 0, lastX = -1, deltaHaut = 100;
		if(!isConsistent)
			indexB = -1;
		printf("CONSISTANT : %s", isConsistent ? "OUI" : "NON");
		if(nbTachesHaut > 0 && nbTachesHautBleu > 0) {
			for(int i = 0; i < NB_COULEURS_HAUT; i++) {
				if(isConsistent) {
					if(!(indexB >= nbTachesHautBleu)) {
						if(tachesHautBleu.at(indexB).x <= tachesHaut.at(indexR).x) {
							couleurTachesHaut[i] = BLEU;
						}
						indexB++;
					} else {
						couleurTachesHaut[i] = ROUGE;
						indexR++;
					}
				} else {
					printf("IndexB=%d, IndexR=%d\n", indexB, indexR);
					if(indexB == -1) {
						if(tachesHautBleu.at(0).x > tachesHaut.at(0).x) {
							// Signifie qu'on n'a pas reçu la tache la plus à gauche, bleue, il faut donc l'ajouter
							couleurTachesHaut[i] = BLEU;
						}
						indexB = 0;
					}
					if(!(indexB >= nbTachesHautBleu)) {
						if((lastX == -1 || abs(lastX + deltaHaut - tachesHautBleu.at(indexB).x) < 5) &&
						   tachesHautBleu.at(indexB).x <= tachesHaut.at(indexR).x) {
							// Situation normale, bleu avant rouge
							couleurTachesHaut[i] = BLEU;
							lastX = tachesHautBleu.at(indexB).x;
							indexB++;
						}
					} else if(!(indexR >= nbTachesHaut)) {
						if((lastX == -1 || abs(lastX + deltaHaut - tachesHaut.at(indexR).x) < 5) &&
						   tachesHautBleu.at(indexB).x > tachesHaut.at(indexR).x) {
							// Situation normale, rouge avant bleu
							couleurTachesHaut[i] = ROUGE;
							lastX = tachesHaut.at(indexR).x;
							indexR++;
						}
					} else {
						couleurTachesHaut[i] = UNDEFINED;
		...
		//// BAS ////
		printf("Calcul des tâches pour le BAS\n");
		/// TODO : /!\ BLANCHES
		nbTachesBasBleu = tachesBasBleu.size();
		nbTachesBas = tachesBas.size();
		printf("TachesBleues : %d, tachesRouges : %d\n", nbTachesBasBleu, nbTachesBas);
		isConsistent = (nbTachesBasBleu + nbTachesBas == NB_COULEURS_BAS);
		indexB = 0;
		indexR = 0;
		lastX = -1;
		int deltaBas = 70;
		if(!isConsistent)
			indexB = -1;
		printf("CONSISTANT : %s", isConsistent ? "OUI" : "NON");
		if(nbTachesBas > 0 && nbTachesBasBleu > 0) {
			for(int i = 0; i < NB_COULEURS_BAS; i++) {
				if(isConsistent) {
					if(!(indexB >= nbTachesBasBleu)) {
						if(tachesBasBleu.at(indexB).x <= tachesBas.at(indexR).x) {
							couleurTachesBas[i] = BLEU;
						}
						indexB++;
					} else {
						couleurTachesBas[i] = ROUGE;
						indexR++;
					}
				} else {
					printf("IndexB=%d, IndexR=%d\n", indexB, indexR);
					if(indexB == -1) {
						if(tachesBasBleu.at(0).x > tachesBas.at(0).x) {
							// Signifie qu'on n'a pas reçu la tache la plus à gauche, bleue, il faut donc l'ajouter
							couleurTachesBas[i] = BLEU;
						}
						indexB = 0;
					...
					if(!(indexB >= nbTachesBasBleu)) {
						if((lastX == -1 || abs(lastX + deltaBas - tachesBasBleu.at(indexB).x) < 15) &&
						   tachesBasBleu.at(indexB).x <= tachesBas.at(indexR).x) {
							// Situation normale, bleu avant rouge
							couleurTachesBas[i] = BLEU;
							lastX = tachesBasBleu.at(indexB).x;
							indexB++;
						}
					} else if(!(indexR >= nbTachesBas)) {
						if((lastX == -1 || abs(lastX + deltaBas - tachesBas.at(indexR).x) < 15) &&
						   tachesBasBleu.at(indexB).x > tachesBas.at(indexR).x) {
							// Situation normale, rouge avant bleu
							couleurTachesBas[i] = ROUGE;
							lastX = tachesBas.at(indexR).x;
							indexR++;
						}
					} else {
						couleurTachesBas[i] = UNDEFINED;
		...
		//		int index = 0;
		//		int deltaBas = 70;
		//		if (isBlue) startXBBlue = tachesBas[index].x;
		//		for (int i=0; i<NB_COULEURS_BAS; i++) {
		//			// Ecartement théorique entre deux bougies du bas : 70px (offset de 20)
		//			if (abs(tachesBas[index].x - (startXBBlue + i*deltaBas)) < 15 || (tachesHaut[index].x > (startXBBlue
		//+
		//(i-1)*deltaBas) && tachesBas[index].x < (startXBBlue + i*deltaBas))) {
		...
		for(int i = 0; i < NB_COULEURS_HAUT; i++) {
			...
			//			}
			//			else if ((int)couleurTachesHautBleu[i] != (int)couleurTachesHaut[i] && nbTachesHautBleu >
			// nbTachesHaut) {
			...
			// couleurToString(couleurTachesHautBleu[i]), couleurToString(couleurTachesHaut[i]));
			//			}
			//			else {
		...
		for(int i = 0; i < NB_COULEURS_BAS; i++) {
			...
			//			}
			//			else if ((int)couleurTachesBasBleu[i] != (int)couleurTachesBas[i] && nbTachesBasBleu >
			// nbTachesBas)
			//{
