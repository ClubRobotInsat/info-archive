...
#define GETBITS(in, n) \
	((le < (n) ? le = fillbits(in, le, bi), bi = in->bits : 0), (le -= (n)), bi >> le & ((1 << (n)) - 1))
...
static unsigned char zig2[64] = {0,  2,  3,  9,  10, 20, 21, 35, 14, 16, 25, 31, 39, 46, 50, 57, 5,  7,  12, 18, 23, 33,
                                 37, 48, 27, 29, 41, 44, 52, 55, 59, 62, 15, 26, 30, 40, 45, 51, 56, 58, 1,  4,  8,  11,
...
static unsigned char zig[64] = {0,  1,  5,  6,  14, 15, 27, 28, 2,  4,  7,  13, 16, 26, 29, 42, 3,  8,  12, 17, 25, 30,
...
static PREC aaidct[8] = {IFIX(0.3535533906),
                         IFIX(0.4903926402),
                         IFIX(0.4619397663),
                         IFIX(0.4157348062),
                         IFIX(0.3535533906),
	...
	if(file != NULL) {
		if(!is_huffman(buf)) {
			...
			while(((ptcur[0] << 8) | ptcur[1]) != 0xffc0)
				ptcur++;
			sizein = ptcur - ptdeb;
			dummy = fwrite(buf, sizein, 1, file);
			dummy = fwrite(dht_data, DHT_SIZE, 1, file);
			dummy = fwrite(ptcur, size - sizein, 1, file);
		} else {
			dummy = fwrite(ptcur, size, 1, file); /* ptcur was uninit -wsr */
		}
		fclose(file);
	}
	if(name)
...
int get_pictureYV2(unsigned char* buf, int width, int height) {
	...
	if(name) {
		...
		fprintf(foutpict, "P6\n%d %d\n255\n", width, height);
		dummy = fwrite(picture, sizeof(char), width * height * 3, foutpict);
		fclose(foutpict);
		free(name);
	}
	free(picture);
