...
static inline int vm_insert_page(struct vm_area_struct* vma, unsigned long addr, struct page* page) {
	...
	return remap_pfn_range(vma, addr, page_to_pfn(page), PAGE_SIZE, vma->vm_page_prot);
}
#endif
 ...
 * v4l_printk_ioctl()
 */
static inline void v4l_printk_ioctl(unsigned int cmd) {
