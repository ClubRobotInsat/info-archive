...
int init_videoIn(struct vdIn* vd, const char* device, int width, int height, float fps, int format, int grabmethod, const char* avifilename) {
	int ret = -1;
	int i;
	if(vd == NULL || device == NULL)
		return -1;
	...
	vd->toggleAvi = 0;
	vd->avifile = NULL;
	vd->avifilename = (char*)avifilename;
	vd->recordtime = 0;
	vd->framecount = 0;
	vd->recordstart = 0;
	vd->getPict = 0;
...
static int init_v4l2(struct vdIn* vd) {
	...
	if(requested_format_found) {
		...
		printf("  Frame format: " FOURCC_FORMAT " (" FOURCC_FORMAT " is not supported by device)\n",
		       FOURCC_ARGS(device_formats[0]),
		       FOURCC_ARGS(vd->formatIn));
	...
	if(ret == 0) {
		float confirmed_fps =
		    (float)setfps->parm.capture.timeperframe.denominator / (float)setfps->parm.capture.timeperframe.numerator;
