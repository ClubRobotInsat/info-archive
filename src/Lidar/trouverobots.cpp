...
#define SPIRAL(center, maxRad, body)  \
	...
	std::vector<char> scanned(tp.x * tp.y);

	// printf("\ndébut\n");

	// les candidats passés 'vieillissent'
	int tc = _candidats.size();
	for(int i = 0; i < tc; ++i) {
		int cWin = 0;

		IVec2 pCand = {round((_candidats[i].x / ech.x) * tp.x), round((_candidats[i].y / ech.y) * tp.y)};
			...
			if(pos.x >= 0 && pos.x < tp.x && pos.y >= 0 && pos.y < tp.y) {
				if(!scanned[ind]) {
					...
					if(r < MAX_MOVE && grid(pos.x, pos.y)) { // +1 point vers la victoire
						...
						_candidats[i] *= 1 - (1 / (double)cWin);
						_candidats[i] +=
						    (1 / (double)cWin) * Vec2((pos.x / (double)tp.x) * ech.x, (pos.y / (double)tp.y) * ech.y);
...
std::vector<Vec2> TrouveRobots::getResults() const {
	std::vector<Vec2> res;
