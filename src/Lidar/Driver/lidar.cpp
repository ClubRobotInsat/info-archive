...
namespace Lidar {
	void FrameLidar::save_to_file(std::string const& file) {
		...
		out << (angular_resolution * 1e6).toRad<uint32_t>() << '\n';
	...
	FrameLidar::FrameLidar(std::string const& filename) {
		...
		in >> tmp;
		angular_resolution = Angle::makeFromMilliRad(tmp) / 1000;
		in >> stmp;
...
#define EP_TOLIDAR 0x02
	...
	std::unique_ptr<Lidar> Lidar::open_lidar(Type lidarType) {
		...
		if(sick && lidarType != Type::Hokuyo) {
			libusb_device_handle* hnd = nullptr;
			int rc = libusb_open(sick, &hnd);
...
#undef TRY
	...
	/*void Lidar::sendData(uint8_t const *data, size_t dataLength) {
	    ...
	    do {
	        int rc = libusb_bulk_transfer(_hDev, EP_TOLIDAR, const_cast<uint8_t *>(data) + sent, dataLength - sent, &w,
	0); if(rc) throw Erreur("USB Write failed"); sent += w; } while(sent < dataLength);
	}
