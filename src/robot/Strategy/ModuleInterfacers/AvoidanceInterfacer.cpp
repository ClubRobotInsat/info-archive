...
namespace Strategy::Interfacer {
	...
	std::vector<int> AvoidanceInterfacer::update_environment(Environment& env) const {
		...
		const Distance adv_radius = 40_cm; // 0.80f * env.getRobotRadius();
	...
	void AvoidanceInterfacer::thread_lidar_main() {
		std::unique_ptr<OccupGrid> lidar_map = std::make_unique<OccupGrid>(toVec2(GLOBAL_CONSTANTS().get_table_size()), 100, 66);
