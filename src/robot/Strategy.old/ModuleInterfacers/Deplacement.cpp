...
namespace Commun {
	...
	Deplacement::Deplacement(Robot& robot, Environment& env, Evitement& evitement)
	        : _robot(robot), _evitement(evitement), _module(robot.get_module_move()), _env(env), _suiviTrajectoire(1, "Suivi trajectoire") {
	...
	ResultatAction Deplacement::avancerDe(Distance distance, SensAvance sens, Duration timeout) {
	    ...
	    auto posInitiale = _robot.read_coordinates().getPos2D(ref);
	    auto angleInitial = _robot.read_coordinates().getAngle(ref);
	    ...
	    do {
	        sleep(_carteDeplacement.getPeriodeEnvoiAuto());
	        auto currentPos = _robot.read_coordinates().getPos2D(ref);
	...
	Distance Deplacement::computeBackupDistance(SensAvance sens, Distance escapeRadius) {
	    ...
	    Coordonnees coords = _robot.read_coordinates();
	...
	ResultatAction Deplacement::tournerAbsolu(Angle angle, Duration timeout) {
	    SensRotation sens =
	        sensRotationOptimal({_robot.read_coordinates().getAngle()}, {angle, _robot.getStrategie().getReference()});
	...
	ResultatAction Deplacement::pointerVers(Distance x, Distance y, Duration timeout) {
	    return pointerVers(x, y, sensRotationOptimal(_robot.read_coordinates().getAngle(), Vector2m(x, y).angle()), timeout);
	}
	...
	Angle Deplacement::deltaCap(Vector2m const& cible) {
	    return (cible - _robot.read_coordinates().getPos2D()).angle();
	}
	...
	std::function<bool()> Deplacement::testDeplacementUnitaireTermine() {
		return [this]() {
			return true; /*return _carteDeplacement.verifierDeplacementTermine();*/
		};
	}

	std::function<bool()> Deplacement::testDeplacementUnitairePrecisionAtteinte() {
		return [this]() {
			return true; /*return _carteDeplacement.verifierPrecisionAtteinte();*/
		};
	}
	...
	typename Deplacement::Trajectoire
		...
		Coordonnees origin = this->_robot.read_coordinates();
