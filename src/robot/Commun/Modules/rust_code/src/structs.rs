...
pub struct SharedServos2019 {
    pub servos: [Servo2019; 8/*NBR_SERVOS as usize*/],
...
extern "C" {
    pub fn servo_read_frame(message: *const libc::uint8_t, size: libc::uint8_t) -> SharedServos2019;
    pub fn servo_write_frame(buf: *mut libc::uint8_t, buf_size: libc::uint8_t, obj: *const SharedServos2019) -> libc::uint8_t;

    // fn motor_read_frame(message: *const libc::uint8_t, size: libc::uint8_t) -> SharedMotors2019;
    // fn motor_write_frame(buf: *mut libc::uint8_t, buf_size: libc::uint8_t, obj: *const SharedMotors2019) -> libc::uint8_t;
...
pub fn generic_read_frame<T>(message: Vec<u8>, c_read_function: unsafe extern "C" fn(*const uint8_t, uint8_t) -> T) -> Result<T, ErrorParsing>
    where T: FrameParsingTrait,
          T: TypeInfo
{
    ...
    if servo.read_is_ok() {
        ...
        println!("Reading {}'s struct failed: error during its creation.", servo.type_of());
        Err(ErrorParsing::BadPadding)
    }
}

fn generic_write_frame<T>(obj: &T, c_write_function: unsafe extern "C" fn(*mut uint8_t, uint8_t, *const T) -> uint8_t) -> Result<Vec<u8>, ErrorParsing>
    where T: TypeInfo
{
    ...
    if size == 0 {
        println!("Building {}'s frame failed: not enough space.", obj.type_of());
...
pub trait FrameParsingTrait {
    fn read_frame(_msg: Vec<u8>) -> Result<Self, ErrorParsing> where Self: Sized;
