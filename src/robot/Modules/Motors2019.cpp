...
namespace PhysicalRobot {
	...
	uint8_t Motors2019::test_is_controlled_ok(uint8_t id) const {
		...
		if(index >= NB_MAX_CONTROLLED_MOTORS) {
	...
	uint8_t Motors2019::test_is_uncontrolled_ok(uint8_t id) const {
		...
		if(index >= NB_MAX_UNCONTROLLED_MOTORS) {
	...
	uint8_t Motors2019::get_index_of_controlled(uint8_t id) const {
		const uint8_t INDEX_BAD_ID = NB_MAX_CONTROLLED_MOTORS;
		std::lock_guard<std::mutex> lk(_mutex_variables);
		...
		for(uint8_t index = 0; index < NB_MAX_CONTROLLED_MOTORS; ++index) {
	...
	uint8_t Motors2019::get_index_of_uncontrolled(uint8_t id) const {
		const uint8_t INDEX_BAD_ID = NB_MAX_UNCONTROLLED_MOTORS;
		std::lock_guard<std::mutex> lk(_mutex_variables);
		...
		for(uint8_t index = 0; index < NB_MAX_UNCONTROLLED_MOTORS; ++index) {
	...
	uint8_t Motors2019::get_index_of_brushless(uint8_t id) const {
		const uint8_t INDEX_BAD_ID = NB_MAX_BRUSHLESS;
		std::lock_guard<std::mutex> lk(_mutex_variables);
		...
		for(uint8_t index = 0; index < NB_MAX_BRUSHLESS; ++index) {
	...
	void Motors2019::add_uncontrolled_motor(uint8_t id) {
		if(get_nbr_uncontrolled() >= NB_MAX_UNCONTROLLED_MOTORS) {
			throw std::runtime_error("Trop de moteurs non-asservis ont été défini !");
		} else if(id == 0) {
			throw std::runtime_error("L'ID 0 des moteurs est réservé !");
		} else if(get_index_of_uncontrolled(id) < NB_MAX_UNCONTROLLED_MOTORS) {
	...
	void Motors2019::add_brushless(uint8_t id) {
		if(get_nbr_brushless() >= NB_MAX_BRUSHLESS) {
