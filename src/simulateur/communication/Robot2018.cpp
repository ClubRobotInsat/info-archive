...
const Mass Robot2018::MASSE_ROBOT = 10_kg;

/****************
 * CONSTRUCTEUR *
 ****************/
// FIXME : pouvoir choisir le port, là c'est dégueu
Robot2018::Robot2018(const std::string& connexion, RobotColor col)
	...
	this->_tcpServer = std::thread(std::bind(&Robot2018::listenSocket, this));
}
...
void Robot2018::update(Duration elapsed) {
	Base::update(elapsed);
}

void Robot2018::setPhysicalObject(IPhysicalInstance* object) {
	_obj = object;
 ...
 */
void Robot2018::setEngineAngularSpeed(uint8_t /*idCarte*/, AngularSpeed /*speed*/) {}
Angle Robot2018::getEngineAngle(uint8_t /*idCarte*/) {
	return 0_rad;
}
bool Robot2018::isBlocked(uint8_t /*idCarte*/) const {
	return true;
}
 ...
 */
int Robot2018::getNumberofContactor() {
	return sizeof(_etatContacteur);
}

bool& Robot2018::getContactor(int contactorID) {
...
Constantes::RobotColor Robot2018::getColor() {
	return _color;
}